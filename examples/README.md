...
Current released entry points:

- `phi4/`
  - `phi4_re_scan.py`: reproducible replica-exchange GG-PA scans for the 2D Ginzburg-Landau phi^4 lattice.
...
- `alanine_dipeptide/`
  ...
  - `run_ad_sodium.py`: zero-shot AD-Na+ runner; defaults to the `5 x 5000` ensemble and supports `--single-run` for debugging.
  - `run_ad_dimer.py`: replica-exchange GG-PA example for the alanine dipeptide dimer.
  - `run_ad_dimer.sh`: background launcher for the AD dimer GG-PA-RE example plus automatic plotting.
