...
This folder contains two released GG-PA examples driven by the shared torsion
checkpoint `checkpoints/ad_torsion_prior.pt`.

## Included workflows

- `run_ad_sodium.py`
  - zero-shot `AD-Na+` example built around the bundled `0%` torsion prior
...
- `run_ad_dimer.py`
  - AD dimer example using diffusion-time replica exchange
  ...
  - saves full noisy observables for every replica plus two comparison figures:
  ...
  - a single run may spontaneously select one of the two symmetry-related parallel branches,
    so `P_LR` vs `P_RL` asymmetry in a single generated figure is expected rather than a bug

- `run_ad_dimer.sh`
  - one-command background launcher for the AD dimer GG-PA-RE example
  - runs the simulation and then regenerates the figures
...
- `run_ad_sodium.sh`
  - one-command background launcher for the AD-Na+ zero-shot ensemble
...
The default released config uses `device: cpu` together with `platform_name: CUDA`.
...
For the AD-Na+ example, the default config reflects the more stable
zero-shot setting:
...
For the AD dimer example, the default config reflects the tested GG-PA-RE setup:
...
- on a single modern GPU workstation this run takes about `2 min`
...
Run the AD-Na+ zero-shot ensemble in the foreground:
...
Run the single-trajectory AD-Na+ debug entry:
