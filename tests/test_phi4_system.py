...
def test_gaussian_context_log_prob_matches_full_fft_quadratic_form(L):
    ...
    a2, s2 = _vp_params_at_t_diff(scheduler, t_diff)
    ...
    assert np.isclose(ctx.log_prob(s, t_diff), expected, rtol=0.0, atol=1e-10)
...
def test_phi4_context_gradients_match_finite_difference(context_factory, t_diff):
    scheduler = DummyNoiseScheduler()
    L = 6
    s = np.random.default_rng(2026).standard_normal((L, L))
    ctx = context_factory(scheduler, L)
    grad = np.asarray(ctx.grad_log_prob(s, t_diff), dtype=np.float64)

    for idx in [(0, 0), (1, 2), (3, 5)]:
        fd = _finite_difference(lambda x: ctx.log_prob(x, t_diff), s, idx)
