...
class UniformContext(ContextBase):
    ...
    def log_prob(self, s: np.ndarray, t_diff: float) -> float:
        """Log probability (constant 0, independent of t_diff)."""
        return 0.0
    
    def grad_log_prob(self, s: np.ndarray, t_diff: float) -> np.ndarray:
