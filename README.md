...
### Option 1: Run the quick notebooks without installing the package

The notebooks are set up to add `src/` to `sys.path` automatically when launched from the repository root or from `notebooks/`.

1. Create the environment with either `environment.yml` or `requirements.txt`:
...
2. The default `torch` entry in these files is CPU-safe. For a CUDA-specific build, replace it with the command generated by the official selector at https://pytorch.org/get-started/locally/.
3. These default environment files now also include the dependencies needed by the public alanine-dipeptide examples (`openmm` and `mdtraj`).
4. Then open:

- `notebooks/example_doublewell.ipynb`
- `notebooks/example_phi4.ipynb`

### Option 2: Install `ggpa` as a package

If you want to import the package directly, use the same environment and install the repo in editable mode:

```bash
python -m pip install -e .
```

If you want the package extras to install the notebook dependencies as well:

```bash
python -m pip install -e ".[torch,notebooks]"
```

For the alanine-dipeptide / OpenMM workflow, the additional optional dependencies are already included in the public `requirements.txt` / `environment.yml`. If you want to install them through package extras instead, use:

```bash
python -m pip install -e ".[alanine]"
```
...
```python
import ggpa
print(ggpa.__version__)
```

Torch-backed model import:
...
This repository now includes a small smoke-test suite focused on packaging and public quick-run assets rather than exhaustive scientific validation.
...
If this repository is useful in your work, please cite the associated paper. A finalized citation entry will be added once the paper metadata is public.
